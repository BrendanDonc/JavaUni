...
/**
 * The 'Item' class is used for storing information about the various items that
 * populate the inventory management application.
 * 
 * @author Mary Millar
 *
 */
public class Item {
    ...
    /**
     * A constructor that's used to declare an item that does not require
     * temperature restricted storage.
     * 
     * @param itemName
     *            The name of the item.
     * @param itemCost
     *            The manufacturing cost of the item.
     * @param itemPrice
     *            The sale price of the item.
     * @param itemReorderPoint
     *            The amount current stock of an item must fall under for more of
     *            the item to be reordered.
     * @param itemReorderAmount
     *            The amount that is reordered once an item falls under the reorder
     *            point.
     * 
     * @throws StockException
     *             The exception thrown if: the item cost is less than zero, the
     *             item price is less than zero, the item reorder point is less than
     *             zero, or the item reorder amount is less than zero.
     */
    ...
    /**
     * A constructor that's used to declare an item that does require temperature
     * restricted storage.
     * 
     * @param itemName
     *            The name of the item.
     ...
     *            point.
     * @param itemTemp
     *            The temperature required for the item throughout the supply chain
     *            proccess.
     * 
     * @throws StockException
     *             The exception thrown if: the item cost is less than zero, the
     *             item price is less than zero, the item reorder point is less than
     *             zero, the item reorder amount is less than zero, or the
     *             temperature is less than -20 or more than 10 degrees Celsius.
     */
    public Item(String itemName, double itemCost, double itemPrice, int itemReorderPoint, int itemReorderAmount,
            Double itemTemp) throws StockException {
        if (itemCost <= 0 || itemPrice <= 0 || itemReorderPoint <= 0 || itemReorderAmount <= 0
                || !(-20 <= itemTemp && itemTemp <= 10)) {
            throw new StockException("The item '" + itemName + "' has a bad parameter");
        } else {
            this.name = itemName;
            this.cost = itemCost;
    ...
    /**
     * This method is used to determine if the item needs to be
     * temperature-controlled.
     * 
     * @return A boolean representing if the item needs to be temperature-controlled
     *         (true) or not (false).
     */
    ...
    /**
     * This method compares each parameter of the current item against the input to
     * determine if they equal.
     * 
     * @param item
     *            The item that needs to be compared.
     * 
     * @return A boolean representing if both items are identical (true) or not
     *         (false);
     */
