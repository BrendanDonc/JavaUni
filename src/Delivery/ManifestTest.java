...
public class ManifestTest {
    ...
    public void returnTruckManifest() {
        manifestArray.add(truckOrdinary);
        manifestTest.addTruck(truckOrdinary);
        assertArrayEquals(manifestArray.get(0).getCargo().getItems(),
                manifestTest.returnManifest()[0].getCargo().getItems());
    }

    /* Test 5: Convert Manifest into 'printable' format (formatted) */
    @Test
    public void printManifestTest() {
        String stringResult = ">Ordinary\n" + "rice,50\n" + "biscuits,30\n";
        manifestTest.addTruck(truckOrdinary);
    ...
    /*
     * Test 7: Import manifest from a .csv file (we're converting .csv to strings so
     * just use string inputs - see if test 3 matches to "manual" input/test)
     */
    @Test
